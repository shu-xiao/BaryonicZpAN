...
struct tri{
    int nPass = 0;
    TString triName;
    tri(string name,int w) {
        triName = name;
        nPass = w;
    }
};
bool sortEffi(tri a, tri b) {return a.nPass>b.nPass;}
...
using namespace std;
void anbb2HDM_4jet_sel(int w=0, std::string inputFile="../reco4jets/2HDMfullSimFile/2HDM_MZp600_MA0400.root"){
    ...
    TH1F* h_triEffi  = new TH1F("h_triEffi","h_triEffi",20,0,20);
    ...
    TH1F* h_HTtriTotal = new TH1F("h_HTtriTotal","h_HTtriTotal",40,0,1000);
    TH1F* h_HTtriPass = new TH1F("h_HTtriPass","h_HTtriPass",40,0,1000);
    h_HTtriTotal->Sumw2();
    h_HTtriPass->Sumw2();
    ...
    TTree *tree;
    if (saveTree) {
        if (isBG) ftree = new TFile(Form("treeBG_%d.root",w),"recreate");
        else ftree = new TFile("treeSig.root","recreate");
    ...
    vector<int> triggerCout;
    vector<tri> triggervec;
    for(Long64_t jEntry=0; jEntry<data.GetEntriesFast() ;jEntry++){
        ...
        for(unsigned int it=0; it< trigResult.size(); it++) {
            ...
            for (unsigned int j=0;j<triggerList.size();j++) {
                if (!calTriAll) break;
                //if (!trigResult[it]) continue;
                if (triggerList[j].EqualTo(thisTrig.data())) {
                    triggerCout[j]+=trigResult[it];
                    isnew =false;
                    break;
                }
            }
            */
            for (unsigned int j=0;j<triggervec.size();j++) {
                if (!calTriAll) break;
                if (!results) continue;
                if (triggervec[j].triName.EqualTo(thisTrig.data())) {
                    triggervec[j].nPass++;
                    isnew =false;
                    break;
                }
        ...
        for (int i=0;i<nGenJet;i++) {
            ...
            // select TMML
            if (isM)  jPass = CISVV2[i]>CISVV2CUT_M;
            else      jPass = CISVV2[i]>CISVV2CUT_T;
            if (nSeljet<4&&jPass) {
                Hind[0][nSeljet] = i;
                nSeljet++;
    ...
    } // end of loop over entris
    TGraphAsymmErrors *h_effiTriHT = new TGraphAsymmErrors(h_HTtriPass,h_HTtriTotal,"cl=0.683 b(1,1) mode");
    //TGraphAsymmErrors *h_effiTriHT = new TGraphAsymmErrors(h_HTtriPass,h_HTtriTotal);
    ...
    if (!isBG&&false) {
        ...
        h_HTtriPass->SetLineColor(kBlue);
        h_HTtriPass->Draw("histsame");
        c1->Print(pdfName.data());
        h_NbJets->Draw("hist");
        c1->Print(pdfName.data());
    ...
    string fileName; 
    if (!isBG) {
        string txtName = Form("triEffi_criteria%d/triEffi_MZp%d_MA0%d.txt",isM,Zpmass.Atoi(),A0mass.Atoi());
        //string txtName = Form("triEffi_T/triEffi_MZp%d_MA0%d.txt",Zpmass.Atoi(),A0mass.Atoi());
        const int len = 10;
        const int init = 4;
        int np[len];
        int np2[len];
        for (int i = 0;i<len;i++) np[i] = 0;
        for (int i = 0;i<len;i++) np2[i] = 0;
        for (int i=0;i<=len-1;i++) {
            np[i] = nPass[i+init];
            np2[i] = nPass_anti[i+init];
        }
