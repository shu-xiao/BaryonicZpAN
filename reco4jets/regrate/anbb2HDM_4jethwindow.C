...
using namespace std;
void anbb2HDM_4jethwindow(int w=0, std::string inputFile="2HDM_MZp1000_MA0300_re.root"){
...
# if doGenMatch != 0 || corr != 0
    ...
    TH1F* h_Ncisvv2Num = new TH1F("h_Ncisvv2Num","h_Ncisvv2Num",50,-50,500);
    // Mass
    ...
    TTree *tree;
    if (saveTree) {
        ftree = new TFile(Form("treehWindow_%d.root",w),"recreate");
...
# endif
        ...
        for (int i=0;i<nGenJet;i++) {
            ...
            // select TMML
            if (nTjet<1&&CISVV2[i]>CISVV2CUT_T) {
                nTjet++;
                LeadHA0Jet.push_back(thisJet);
                LeadHA0_CISVV2.push_back(CISVV2[i]);
                LeadHA0window_CISVV2.push_back(CISVV2[i]);
            }
            else if (nMjet<2&&CISVV2[i]>CISVV2CUT_M) {
                nMjet++;
                LeadHA0Jet.push_back(thisJet);
                LeadHA0_CISVV2.push_back(CISVV2[i]);
                LeadHA0window_CISVV2.push_back(CISVV2[i]);
            }
            else if (nLjet<1&&CISVV2[i]>CISVV2CUT_L) {
                nLjet++;
                LeadHA0Jet.push_back(thisJet);
                LeadHA0_CISVV2.push_back(CISVV2[i]);
                LeadHA0window_CISVV2.push_back(CISVV2[i]);
        ...
        if ((nCisvv2_L+nCisvv2_M+nCisvv2_T)>=4) {
            if (nCisvv2_L>=10) h_Ncisvv2Num->Fill(-10);
            if (nCisvv2_M>=10) h_Ncisvv2Num->Fill(-20);
            if (nCisvv2_T>=10) h_Ncisvv2Num->Fill(-30);
            h_Ncisvv2Num->Fill(nCisvv2_T*100+nCisvv2_M*10+nCisvv2_L);
        }
        ...
        for (int i=0,k=0,g=0;i<4;i++) {
            for (int j=0;j<i;j++) {
                ...
                if (abs(hj.M()-125)<mindeMH) {
                    Hind[1][0] = i; Hind[1][1] = j;
                    mindeMH = abs(hj.M()-125);
                }
           }
        }
        if (inWindow) nPass[4]++;
        if (inWindow) arrangeIndex(Hind[0][0],Hind[0][1],Hind[0][2],Hind[0][3]);
        arrangeIndex(Hind[1][0],Hind[1][1],Hind[1][2],Hind[1][3]);
        // convert index to jet
        //cout << inWindow << "\t" << Hind[0][0] << "\t" << Hind[0][1] << "\t" << Hind[0][2] << "\t" << Hind[0][3] << endl;
        // swap h and a0
        for (int j=0;j<4;j++) LeadSelHA0Jet[1][j] = LeadHA0Jet[Hind[1][j]]; 
        LeadSelHA0Jet[1][4] = new TLorentzVector(*LeadSelHA0Jet[1][0]+*LeadSelHA0Jet[1][1]);
        LeadSelHA0Jet[1][5] = new TLorentzVector(*LeadSelHA0Jet[1][2]+*LeadSelHA0Jet[1][3]);
        if (LeadSelHA0Jet[1][4]->M()>LeadSelHA0Jet[1][5]->M()) {
            swap(LeadSelHA0Jet[1][0],LeadSelHA0Jet[1][2]);
            swap(LeadSelHA0Jet[1][1],LeadSelHA0Jet[1][3]);
            swap(LeadSelHA0Jet[1][4],LeadSelHA0Jet[1][5]);
            swap(Hind[1][0],Hind[1][2]);
            swap(Hind[1][1],Hind[1][3]);
            swap(LeadHA0_CISVV2[0],LeadHA0_CISVV2[2]);
            swap(LeadHA0_CISVV2[1],LeadHA0_CISVV2[3]);
        }
        if (inWindow) {
            ...
            if (LeadSelHA0Jet[0][4]->M()>LeadSelHA0Jet[0][5]->M()) {
                ...
                swap(LeadHA0window_CISVV2[0],LeadHA0window_CISVV2[2]);
                swap(LeadHA0window_CISVV2[1],LeadHA0window_CISVV2[3]);
            }
        }
        // fill hist
        h_hM->Fill(LeadSelHA0Jet[1][4]->M());
        h_hPt->Fill(LeadSelHA0Jet[1][4]->Pt());
        h_hDeltaR->Fill(LeadSelHA0Jet[1][0]->DeltaR(*LeadSelHA0Jet[1][1]));
        h_hDeltaEta->Fill(abs(LeadSelHA0Jet[1][0]->Eta()-LeadSelHA0Jet[1][1]->Eta()));
        h_hDeltaPhi->Fill(LeadSelHA0Jet[1][0]->DeltaPhi(*LeadSelHA0Jet[1][1]));
        h_hPtAs->Fill(ptAssymetry(LeadSelHA0Jet[1][0],LeadSelHA0Jet[1][1]));
        h_hPtSD->Fill(softDropAs(LeadSelHA0Jet[1][0],LeadSelHA0Jet[1][1]));
        // end of filling
        CISVV2_Hb1 = LeadHA0_CISVV2[0];
        CISVV2_Hb2 = LeadHA0_CISVV2[1];
        CISVV2_A0b1 = LeadHA0_CISVV2[2];
        CISVV2_A0b2 = LeadHA0_CISVV2[3];
        Mh = (*LeadSelHA0Jet[1][0]+*LeadSelHA0Jet[1][1]).M();
        hPt = (*LeadSelHA0Jet[1][0]+*LeadSelHA0Jet[1][1]).Pt();
        hDeltaR = LeadSelHA0Jet[1][0]->DeltaR(*LeadSelHA0Jet[1][1]);
        hDeltaPhi = LeadSelHA0Jet[1][0]->DeltaPhi(*LeadSelHA0Jet[1][1]);
        hDeltaEta = abs(LeadSelHA0Jet[1][0]->Eta()-LeadSelHA0Jet[1][1]->Eta());
        hptAs = ptAssymetry(LeadSelHA0Jet[1][0],LeadSelHA0Jet[1][1]);
        hsdAs = softDropAs(LeadSelHA0Jet[1][0],LeadSelHA0Jet[1][1]);
        *vh1 = *LeadSelHA0Jet[1][0];
        *vh2 = *LeadSelHA0Jet[1][1];
        *va01 = *LeadSelHA0Jet[1][2];
        *va02 = *LeadSelHA0Jet[1][3];
        /*
        if (LeadHA0Jet.size()==4&&inWindow) {
        ...
        */
        ...
        if (inWindow&&LeadSelHA0Jet[0][4]->M()>LeadSelHA0Jet[0][5]->M()) {
            delete LeadSelHA0Jet[0][4];
            delete LeadSelHA0Jet[0][5];
        }
        if (LeadSelHA0Jet[1][4]->M()>LeadSelHA0Jet[1][5]->M()) {
            delete LeadSelHA0Jet[1][4];
            delete LeadSelHA0Jet[1][5];
        }
