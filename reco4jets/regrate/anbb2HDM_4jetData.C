...
bool findTrigger(string trigName, string triList) {
    return (trigName.find(triList.data())!=std::string::npos);
}
...
using namespace std;
void anbb2HDM_4jetData(int w=0, std::string inputFile="2HDM_MZp1000_MA0300.root"){
    ...
    const float MhMin = 100;
    const float MhMax = 150;
    const float MhSigMin = 115;
    const float MhSigMax = 135;
    TFile *ftree;
    if (saveTree) ftree = new TFile(Form("treeData_%d.root",w),"recreate");
    ...
    TH2F* h_2d3b_MhMA0tag = new TH2F("h_2d3b_MhMA0tag","h_2d3b_MhMA0tag",30,110,140,30,200,500); 
    TH2F* h_2d3b_MhMA0sr = new TH2F("h_2d3b_MhMA0sr","h_2d3b_MhMA0sr",30,110,140,30,200,500); 
    TH2F* h_2d3b_MhMA0cr = new TH2F("h_2d3b_MhMA0cr","h_2d3b_MhMA0cr",30,110,140,30,200,500); 
    TH2F* h_2d2b_MhMA0tag = new TH2F("h_2d2b_MhMA0tag","h_2d2b_MhMA0tag",30,110,140,30,200,500); 
    TH2F* h_2d2b_MhMA0sr = new TH2F("h_2d2b_MhMA0sr","h_2d2b_MhMA0sr",30,110,140,30,200,500); 
    TH2F* h_2d2b_MhMA0cr = new TH2F("h_2d2b_MhMA0cr","h_2d2b_MhMA0cr",30,110,140,30,200,500); 
    ...
    TH1F* h_HT4jet = new TH1F("h_HT4jet","h_HT4jet",40,0,1000);
    ...
    Int_t nTri = 2;
    string triggerList[2] = {"DoubleJet90_Double30_TripleBTagCSV_p087","QuadJet45_TripleBTagCSV_p087"};
    string addtrigger = "HLT_DoubleJet90_Double30_DoubleBTagCSV_p087";
    for(Long64_t jEntry=0; jEntry<data.GetEntriesFast() ;jEntry++){
        ...
        bool pass3bTrigger=false;
        bool pass2bTrigger=false;
        for(unsigned int it=0; it< trigResult.size(); it++) {
            if ((findTrigger(trigName[it],triggerList[0])||findTrigger(trigName[it],triggerList[1]))&&trigResult[it]) {
                pass3bTrigger = true; 
                break;
            }
        }
        if (!pass3bTrigger) {
            for(unsigned int it=0; it< trigResult.size(); it++) {
                if (findTrigger(trigName[it],addtrigger)&&trigResult[it]) {
                    pass2bTrigger = true;
                    break;
                }
            }
        }
        if (!(pass3bTrigger||pass2bTrigger)) continue;
        nPass[2]++;
        ...
        if (isAntiTag) {
            ...
            if (hj.M()>MhMax||hj.M()<MhMin) continue;
            if (hj.M()<MhSigMax&&hj.M()>MhSigMin) insig = true;
            ...
            for (int i=0;i<nGenJet;i++) {
                ...
                if (abs(thisJet->Eta())>2.4) continue;
                if (!vPassID_L[i]) continue;
                if (CISVV2[i]<CISVV2CUT_M&&CISVV2[i]>=0) {
            ...
            for (unsigned int n=0;n<failJet.size();n++) {
                for (int i=0;i<=n/2;i++) {
        ...
        if (isTag) {
            for (int i=0,k=0,g=0;i<4;i++) {
                for (int j=0;j<i;j++) {
                    ...
                    if (abs(hj.M()-125)<mindeMH_range&&hj.M()<MhMax&&hj.M()>MhMin) {
                        if (doswap) {
        ...
        if (isTag) {
            for (int j=0;j<4;j++) HT4jet += LeadSelHA0Jet[0][j]->Pt();
        }
        ...
        // fill hist
        if (isTag) h_HT4jet->Fill(HT4jet);
        if (isTag&&insig) h_2d2b_MhMA0tag->Fill(LeadSelHA0Jet[0][4]->M(),LeadSelHA0Jet[0][5]->M());
        else if (isTag) h_2d2b_MhMA0sr->Fill(LeadSelHA0Jet[0][4]->M(),LeadSelHA0Jet[0][5]->M());
        else if (isAntiTag) h_2d2b_MhMA0cr->Fill(LeadSelHA0Jet[0][4]->M(),LeadSelHA0Jet[0][5]->M());
        if (isTag&&insig&&pass3bTrigger) h_2d3b_MhMA0tag->Fill(LeadSelHA0Jet[0][4]->M(),LeadSelHA0Jet[0][5]->M());
        else if (isTag&&pass3bTrigger) h_2d3b_MhMA0sr->Fill(LeadSelHA0Jet[0][4]->M(),LeadSelHA0Jet[0][5]->M());
        else if (isAntiTag&&pass3bTrigger) h_2d3b_MhMA0cr->Fill(LeadSelHA0Jet[0][4]->M(),LeadSelHA0Jet[0][5]->M());
    ...
    string pdfName = Form("MZp%d_MA0%d_datatest.pdf",Zpmass.Atoi(),A0mass.Atoi());
    if (!isBG&&false) {
