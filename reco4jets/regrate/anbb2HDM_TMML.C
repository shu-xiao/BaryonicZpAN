...
using namespace std;
void anbb2HDM_TMML(int w=0, std::string inputFile="2HDM_MZp1000_MA0300_re.root"){
...
# endif
        ...
        if (nCisL<4) continue;
        nPass[3]++;
        //cout << nCisvv2_L << "  " << nCisvv2_M << "  " << nCisvv2_T <<"\t\t"<< nCisL << "  " << nCisM << "  " << nCisT << endl;
        if (!isMatch) continue;
        nPass[4]++;
        // search LMT index
        if (isMatch) {
    ...
    string nSelText[] = {"nPassSel","min DeltaR","min deMh","min PtAs","max PtAs HA0","max DeltaR HA0"};
    int lastInd = -1;
