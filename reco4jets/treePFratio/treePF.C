...
void treePF(bool doscan = false) {
    ...
    h_Mh[4]->SetMaximum(h_Mh[4]->GetMaximum()*1.4);
    h_Mh[4]->Fit(f1_s);
    h_Mh[4]->Fit(f2_s,"+");
    string fileName = "treePFratioFit.pdf";
    c1->Print((fileName+"[").data());
    doReject = false;
    h_Mh[4]->Draw("e");
    TLegend *leg = new TLegend(0.1,0.77,0.6,0.9);
    ...
    leg->AddEntry(h_Mh[3],"pass");
    leg->AddEntry(h_Mh[5],"fail*p/f (linier)");
    leg->AddEntry(h_Mh[6],"fail*p/f (2nd poly)");
    ...
    for (int ij=0;ij<9;ij++) {
        ...
        for (int i=0;i<t1->GetEntries();i++) {
            ...
            bw = b*f1_s->Eval(Mh[0]);
            b2 = b*f2_s->Eval(Mh[0]);
            h_hPt[5]->Fill(hPt[0],bw);
            h_hDeltaR[5]->Fill(hDeltaR[0],bw);
            h_hDeltaPhi[5]->Fill(hDeltaPhi[0],bw);
            h_hDeltaEta[5]->Fill(hDeltaEta[0],bw);
            h_hptas[5]->Fill(hptas[0],bw);
            h_hsdas[5]->Fill(hsdas[0],bw);
            
            h_hPt[6]->Fill(hPt[0],b2);
            h_hDeltaR[6]->Fill(hDeltaR[0],b2);
            h_hDeltaPhi[6]->Fill(hDeltaPhi[0],b2);
            h_hDeltaEta[6]->Fill(hDeltaEta[0],b2);
            h_hptas[6]->Fill(hptas[0],b2);
            h_hsdas[6]->Fill(hsdas[0],b2);
        }
        f->Close();
        if (!doscan)break;
    } //end of fill
